...
bool Time::waitForValid() { return waitForValid(yac::WallDuration()); }

bool Time::waitForValid(const yac::WallDuration & /*timeout*/) {
  /*yac::WallTime start = ros::WallTime::now();
  while (!isValid() && !stopped_) {
  yac::WallDuration(0.01).sleep();
    if (timeout > yac::WallDuration(0, 0) && (yac::WallTime::now() -
