...
int test_calib_camera_add_camera_data() {
  ...
  MU_CHECK(calib.cam_data.size() > 0);
  MU_CHECK(calib.timestamps.size() > 0);
  MU_CHECK(calib.timestamps.size() == calib.cam_data.size());
...
int test_calib_camera_add_pose() {
  ...
  MU_CHECK(calib.cam_data.count(test_grid.timestamp));
  MU_CHECK(calib.cam_data[test_grid.timestamp].count(0));
...
int test_calib_camera_add_and_remove_view() {
  ...
  for (const auto ts : calib.timestamps) {
    size_t detected = 0;
    for (const auto [cam_idx, grid] : calib.cam_data[ts]) {
  ...
  MU_CHECK(calib.calib_views.size() == 2);
  MU_CHECK(calib.calib_views[0].size() == 1);
  MU_CHECK(calib.calib_views[1].size() == 1);
  MU_CHECK(calib.calib_views[0].count(view_ts));
  MU_CHECK(calib.calib_views[1].count(view_ts));
  ...
  MU_CHECK(calib.calib_views.size() == 2);
  MU_CHECK(calib.calib_views[0].size() == 0);
  MU_CHECK(calib.calib_views[1].size() == 0);
  MU_CHECK(calib.calib_views[0].count(view_ts) == 0);
  MU_CHECK(calib.calib_views[1].count(view_ts) == 0);
  MU_CHECK(calib.problem->NumResidualBlocks() == 0);
...
int test_calib_camera_kalibr_data() {
  ...
  const std::string proj_model = "pinhole";
  const std::string dist_model = "radtan4";
  const vec4_t cam0_proj_params{460.285798, 459.066980,368.299812,244.660910};
  const vec4_t cam0_dist_params{-0.276158,0.067779,0.000757,-0.000299};
  const vec4_t cam1_proj_params{458.931838, 457.621130, 378.783646, 251.341323};
  const vec4_t cam1_dist_params{-0.272847, 0.064882, 0.000506, -0.000210};
  const vec3_t r_C1C0{-0.109948,0.000514,-0.000241};
  const quat_t q_C1C0{0.999970, -0.007147,0.002650,-0.001263};
  const mat4_t T_C1C0 = tf(q_C1C0, r_C1C0);
