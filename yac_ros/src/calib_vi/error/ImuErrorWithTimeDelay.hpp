#ifndef YAC_IMU_ERROR_WITH_TIME_DELAY_HPP
#define YAC_IMU_ERROR_WITH_TIME_DELAY_HPP
...
public:
  ...
  mutable Transformation T_WS_0_last_;
  mutable Transformation T_WS_1_last_;
  mutable SpeedAndBias speedAndBiases_0_last_;
  mutable SpeedAndBias speedAndBiases_1_last_;
  ...
  ImuErrorWithTimeDelay() {}

  ImuErrorWithTimeDelay(const ImuMeasurementDeque &imuMeasurements,
   ...
   */
  void setImuMeasurements(const ImuMeasurementDeque &imuMeasurements) {
   ...
   */
  void setT0(const Time &t_0) { t0_ = t_0; }

  /**
   * (Re)set the start time.
   * @param t_1 End time.
   */
  void setT1(const Time &t_1) { t1_ = t_1; }
   ...
   */
  const ImuParameters &imuParameters() const { return imuParameters_; }

  /**
   * Get the IMU measurements.
   */
  const ImuMeasurementDeque &imuMeasurements() const {
   ...
   * Get the start time.
   */
  Time t0() const { return t0_; }

  /**
   * Get the end time.
   */
  Time t1() const { return t1_; }
    ...
    // Actual propagation output:
    {
      ...
      if (jacobians != NULL) {
        ...
        if (jacobians[4] != NULL) {
          ...
          const ImuErrorWithTimeDelay imu0{imuMeasurements_, imuParameters_, t0_, t1_};
          const ImuErrorWithTimeDelay imu1{imuMeasurements_, imuParameters_, t0_, t1_};
   ...
   */
  virtual std::string typeInfo() const { return "ImuErrorWithTimeDelay"; }
  ...
  ImuMeasurementDeque imuMeasurements_;
...
#endif // YAC_IMU_ERROR_WITH_TIME_DELAY_HPP
