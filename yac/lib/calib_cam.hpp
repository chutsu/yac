...
struct reproj_error_t : public ceres::SizedCostFunction<2, 7, 8> {
...
/* Reprojection Errors */
template <typename CAMERA_TYPE>
void reproj_errors(const calib_data_t &data, std::vector<double> &errs) {
...
struct calib_view_t {
  ...
  void errors(std::vector<double> &errs,
              std::vector<std::pair<int, int>> *tags_meta=nullptr) {
    static_cast<const calib_view_t>(*this).errors(errs, tags_meta);
  }
};
...
void process_grid(const aprilgrid_t &grid,
                  ...
                  ceres::Problem &problem,
                  std::vector<ceres::ResidualBlockId> &res_ids,
                  std::vector<reproj_error_t<T> *> &cost_fns,
                  PoseLocalParameterization &pose_plus) {
  ...
  for (size_t i = 0; i < tag_ids.size(); i++) {
    ...
    auto cost_fn = new reproj_error_t<T>{cam_res, tag_id, corner_idx,
                                                r_FFi, z, covar};
...
// // int calib_camera_covar(const camera_params_t &cam_params,
// //                        ceres::Problem &problem,
// //                        matx_t &covar);
//
...
//  */
// template <typename CAMERA_TYPE>
// int calib_camera_inc_solve(calib_data_t &data) {
//   // Calibration data
...
//   // Build Problem
//   id_t param_id = 0;
//   calib_camera_views_t<CAMERA_TYPE> views;
//   double info = -1;
//   size_t processed = 0;
...
//       matx_t covar;
//       if (calib_camera_covar(cam_params, *problem, covar) == 0) {
