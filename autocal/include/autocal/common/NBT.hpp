#ifndef AUTOCAL_CERES_NBT_HPP
#define AUTOCAL_CERES_NBT_HPP
...
void simulate_imu(const ctraj_t &traj,
                  ...
                  const timestamp_t &ts_end,
                  const mat4_t &T_SC,
                  const autocal::ImuParameters &imu_params,
...
struct nbt_res_blocks_t {
  PoseErrorPtr pose_prior;
  FiducialErrorPtr fiducial_prior;
  TimeDelayErrorPtr timedelay_prior;
  std::map<int, CameraIntrinsicsErrorPtr> cam_priors;
  std::map<int, ExtrinsicsErrorPtr> ext_priors;

  std::vector<ImuErrorPtr> imu_errors;
  std::vector<SpeedAndBiasErrorPtr> sb_errors;
  std::vector<CalibReprojErrorPtr> reproj_errors;
};
