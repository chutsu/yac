...
int solver_t::estimate_covariance_determinant(
  ...
  if (fabs(inv_check) > 1e-4) {
    if (verbose) {
      LOG_WARN("Hmm shape: %ldx%ld", Hmm.rows(), Hmm.cols());
      LOG_WARN("Hmm rank: %ld", spqr.rank());
      LOG_WARN("Covariance estimation: Inverse identity check: %f", inv_check);
      LOG_WARN("This is bad ... Usually means covariance estimated is bad!");
    }
    return -1;
  ...
  // clang-format off
  Eigen::JacobiSVD<matx_t> svd(H_marg, Eigen::ComputeFullU | Eigen::ComputeFullV);
  // -- Set SVD threshold
  const auto nb_rows = H_marg.rows();
  const double eps = std::numeric_limits<double>::epsilon();
  const double tol = eps * nb_rows * svd.singularValues().array().abs()(0);
  svd.setThreshold(tol);
  // -- Get singular values and calculate rank
  const auto rank = svd.rank();
  const vecx_t &s = svd.singularValues();
  covar_det = -1.0 * s.head(rank).array().log().sum();
  // clang-format on
...
void yac_solver_t::_solve_linear_system(const real_t lambda_k,
  ...
  // // dx = linsolve_dense_svd(J, -1.0 * r);
  // dx = linsolve_sparse_qr(J, -1.0 * r);

  // clang-format off
  // tsolver.cholmod_.SPQR_nthreads = 10;
  // const auto eps = std::numeric_limits<double>::epsilon();
  // cholmod_sparse *J_sparse = cholmod_converter_t::convert(-1.0 * J, &tsolver.cholmod_, eps);
  // cholmod_dense *r_dense = cholmod_converter_t::convert(r, &tsolver.cholmod_);
  // tsolver.solve(J_sparse, r_dense, 0, dx);
