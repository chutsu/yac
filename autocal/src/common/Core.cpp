...
// }
//
// double sample_rmse(const std::vector<double> &x) {
//   vecx_t error;
//   error.resize(x.size());
//
...
//   }
//
//   const double sse = error.transpose() * error;
//   const double N = error.size();
...
// };
//
// void save_poses(const std::string &save_path, const mat4s_t &poses) {
//   std::ofstream file{save_path};
//   if (file.good() != true) {
//     printf("Failed to open file for output!");
//     exit(-1);
...
//     auto pos = tf_trans(poses[i]);
//     auto rot = tf_quat(poses[i]);
//     file << rot.w() << ",";
//     file << rot.x() << ",";
//     file << rot.y() << ",";
//     file << rot.z() << ",";
...
// }
//
// void save_keypoints(const std::string &save_path, const vec2s_t &keypoints) {
//   FILE *outfile = fopen(save_path.c_str(), "w");
//   if (outfile == NULL) {
//     FATAL("Failed to open file for output [%s]!", save_path.c_str());
//   }
//
...
//   std::stringstream ss;
//   if (brackets) ss << "[";
// 	ss << v.transpose().format(CSVFormat);
//   if (brackets) ss << "]";
//
//   return ss.str();
//
...
// }
//
// matx_t pinv_symm(const matx_t &A, int *rank) {
//   const Eigen::SelfAdjointEigenSolver<matx_t> eig(A);
// 	const auto eps = std::numeric_limits<double>::epsilon();
//   const auto tol = eps * A.cols() * eig.eigenvalues().array().maxCoeff();
//
//   // printf("A(%ldx%ld)\t", A.rows(), A.cols());
//   // printf("eps: %f\t", tol);
//   // printf("tol: %f\n", tol);
//
// 	const auto Q = eig.eigenvectors();
// 	const auto Lambda = eig.eigenvalues();
// 	const auto lambda_inv = (Lambda.array() > tol).select(Lambda.array().inverse(), 0);
// 	const matx_t Lambda_inv = vecx_t{lambda_inv};
//   const matx_t A_inv = Q * Lambda_inv.asDiagonal() * Q.transpose();
//
//   if (rank) {
//     *rank = 0;
//     for (int i = 0; i < A.rows(); ++i) {
...
//   return (std::max(A.rows(), A.cols()) == rank(A));
// }
//
...
//
//   int rank = 0;
//   const matx_t Hmm_inv = pinv_symm(Hmm, &rank);
//   if (rank != Hmm_inv.rows()) {
//     LOG_WARN("Hmm is rank deficient!");
//     return -1;
//   }
//
//   const double inv_check = ((Hmm * Hmm_inv) - I(m, m)).sum();
...
//
//   // if (full_rank(H_marg) == false) {
//   //   LOG_ERROR("H_marg is not full rank!");
//   //   return -1;
//   // }
//
//   if (debug) {
...
// }
//
// mat4_t lookat(const vec3_t &eye, const vec3_t &target, const vec3_t &up_axis) {
//   // Note: If we were using OpenGL forward would be the opposite direction,
//   // since in OpenGL the camera forward is -z. In robotics however our camera
//   // is +z forward.
//   const vec3_t f = (target - eye).normalized();
//   const vec3_t r = (up_axis.cross(f)).normalized();
//   const vec3_t u = f.cross(r);
//
//   // clang-format off
//   mat4_t A;
//   A << r(0), r(1), r(2), 0.0,
...
//   // clang-format on
//
//   mat4_t T_eye_target = A * B;
//   mat4_t T_target_eye = T_eye_target.inverse();
//   return T_target_eye;
// }
//
...
// }
//
// void tf_flatten(const mat4_t &T, double *data) {
//   vec3_t pos = tf_trans(T);
//   quat_t rot = tf_quat(T);
//
//   data[0] = pos(0);
//   data[1] = pos(1);
//   data[2] = pos(2);
...
//   vec3_t w = (I(3) + axis_skew * (1.0 - cos(rvec_norm)) / rvec_norm + axis_skew * axis_skew * (rvec_norm - sin(rvec_norm)) / rvec_norm) * rvec_deriv;
//
//   return w;
// }
//
...
//     file << ctraj.positions[i](2) << std::endl;
//   }
//
//   // Close file
//   file.close();
...
// }
//
// cv::RotatedRect calc_error_ellipse(const vec2_t &p,
//                                    const mat2_t &covar,
//                                    const int sigma) {
//   Eigen::EigenSolver<Eigen::Matrix<real_t, 2, 2>> eigen_solver(covar);
//   auto eigenvalues = eigen_solver.eigenvalues();
//   auto eigenvectors = eigen_solver.eigenvectors();
//
//   // Calculate the angle between the largest eigenvector and the x-axis
//   double angle = atan2(eigenvectors(0, 1).real(), eigenvectors(0, 0).real());
//
...
//   double chi_sq = 40;
//   // switch (sigma) {
//   //   case 1: chi_sq = 2.27887; break;
//   //   case 2: chi_sq = 5.99146; break;
//   //   case 3: chi_sq = 11.61829; break;
//   //   default: chi_sq = 11.61829; break;
//   // }
//
//   double halfmajoraxissize = chi_sq * sqrt(eigenvalues(0).real());
//   double halfminoraxissize = chi_sq * sqrt(eigenvalues(1).real());
//   if (isnan(halfmajoraxissize)) {
//     printf("det(covar): %f\n", covar.determinant());
//     print_matrix("covar", covar);
//   } else if (isnan(halfminoraxissize)) {
//     printf("det(covar): %f\n", covar.determinant());
//     print_matrix("covar", covar);
//   }
//
...
//   // anti-clockwise
//   cv::Point2f x(p(0), p(1));
//   cv::Size2f ellipse_size(halfmajoraxissize, halfminoraxissize);
//   return cv::RotatedRect(x, ellipse_size, -angle);
// }
//
// cv::Mat draw_error_ellipse(const cv::Mat &image,
//                            const vec2_t &p,
//                            const mat2_t &covar,
//                            const int sigma,
//                            const int radius,
//                            const cv::Scalar &color) {
//   cv::RotatedRect error_ellipse = calc_error_ellipse(p, covar, sigma);
//   cv::ellipse(image, error_ellipse, color, radius);
//   cv::circle(image, cv::Point(p(0), p(1)), radius, color, cv::FILLED);
//   return image;
// }
//
