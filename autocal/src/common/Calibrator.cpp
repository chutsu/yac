...
void load_camera_extrinsics(const config_t &config,
  ...
  parse(config, key, T_BCi);

  if (verbose) {
    LOG_INFO("Sensor-Camera Extrinsics: T_BC%d", cam_idx);
    LOG_INFO("----------------------------------------");
    LOG_INFO("[%f, %f, %f, %f,", T_BCi(0, 0), T_BCi(0, 1), T_BCi(0, 2), T_BCi(0, 3));
    LOG_INFO(" %f, %f, %f, %f,", T_BCi(1, 0), T_BCi(1, 1), T_BCi(1, 2), T_BCi(1, 3));
    LOG_INFO(" %f, %f, %f, %f,", T_BCi(2, 0), T_BCi(2, 1), T_BCi(2, 2), T_BCi(2, 3));
    LOG_INFO(" %f, %f, %f, %f]", T_BCi(3, 0), T_BCi(3, 1), T_BCi(3, 2), T_BCi(3, 3));
    LOG_INFO("");
  }
}
