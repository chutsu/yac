...
void aprilgrid_remove(aprilgrid_t &grid, const int id) {
  ...
  // Remove points
  vec3s_t &points_CF = grid.points_CF;
...
int aprilgrid_grid_index(const aprilgrid_t &grid,
                         ...
                         int &i,
                         int &j) {
	assert(grid.tag_rows > 0);
	assert(grid.tag_cols > 0);
...
void aprilgrid_imshow(const aprilgrid_t &grid,
  ...
  for (size_t i = 0; i < grid.ids.size(); i++) {
    ...
    // Draw corners
    cv::circle(image_rgb, p1, 2, red, -1); // Bottom left
    cv::circle(image_rgb, p2, 2, red, -1); // Top left
    cv::circle(image_rgb, p3, 2, red, -1); // Top right
    cv::circle(image_rgb, p4, 2, red, -1); // Bottom right

    // Draw lines
    cv::line(image_rgb, p1, p2, red, 1);
    cv::line(image_rgb, p2, p3, red, 1);
    cv::line(image_rgb, p3, p4, red, 1);
    cv::line(image_rgb, p4, p1, red, 1);

    // // Draw Tag ID
    // cv::Point2f cxy(p1.x - 10, p1.y + 5);
...
int aprilgrid_detect(aprilgrid_t &grid,
                     ...
                     const bool use_v3) {
  assert(grid.configured);
  if (aprilgrid_detect(grid, detector, image, use_v3) == 0) {
    return 0;
  }
...
bool sort_apriltag_by_id(const apriltag_t &a, const apriltag_t &b) {
  return (a.id < b.id);
}
...
std::ostream &operator<<(std::ostream &os, const aprilgrid_t &grid) {
  ...
  os << "detected: " << grid.detected << std::endl;
  os << "nb_detections: " << grid.nb_detections << std::endl;
