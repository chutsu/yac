...
public:
  ...
  mutable yac::Transformation T_WS_0_last_;
  mutable yac::Transformation T_WS_1_last_;
  mutable yac::SpeedAndBias speedAndBiases_0_last_;
  mutable yac::SpeedAndBias speedAndBiases_1_last_;
  ...
  ImuError2(const yac::ImuMeasurementDeque &imuMeasurements,
            const yac::ImuParameters &imuParameters,
            const yac::Time &t_0,
            const yac::Time &t_1) {
    ...
    //                         "First IMU measurement included in ImuError2 is not "
    //                         "old "
    //                         "enough!");
    // AUTOCAL_ASSERT_TRUE_DBG(Exception,
    //                         t_1 <= imuMeasurements.back().timeStamp,
    //                         "Last IMU measurement included in ImuError2 is not "
    //                         "new "
    //                         "enough!");
  }
  ...
  //  */
  // static int propagation(const yac::ImuMeasurementDeque &imuMeasurements,
  //                        const yac::ImuParameters &imuParams,
  //                        yac::Transformation &T_WS,
  //                        yac::SpeedAndBias &speedAndBiases,
  //                        const yac::Time &t_start,
  //                        const yac::Time &t_end,
  //                        covariance_t *covariance = 0,
  //                        jacobian_t *jacobian = 0) {
  //   // now the propagation
  //   yac::Time time = t_start;
  //   yac::Time end = t_end;
  ...
  //   int i = 0;
  //   for (yac::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin(); it != imuMeasurements.end(); ++it) {
  ...
  //     // Jacobian parts
  //     dalpha_db_g += dt * C_1;
  //     const Eigen::Matrix3d cross_1 = dq.inverse().toRotationMatrix() * cross + yac::rightJacobian(omega_S_true * dt) * dt;
  ...
  int redoPreintegration(
      ...
      const yac::SpeedAndBias &speedAndBiases,
      yac::Time time,
      yac::Time end) const {
    ...
    bool hasStarted = false;
    int i = 0;
    for (yac::ImuMeasurementDeque::const_iterator it =
   ...
   */
  void setImuParameters(const yac::ImuParameters &imuParameters) {
   ...
   */
  void setImuMeasurements(const yac::ImuMeasurementDeque &imuMeasurements) {
   ...
   */
  void setT0(const yac::Time &t_0) { t0_ = t_0; }

  /**
   * (Re)set the start time.
   * @param t_1 End time.
   */
  void setT1(const yac::Time &t_1) { t1_ = t_1; }
   ...
   */
  const yac::ImuParameters &imuParameters() const { return imuParameters_; }

  /**
   * Get the IMU measurements.
   */
  const yac::ImuMeasurementDeque &imuMeasurements() const {
   ...
   * Get the start time.
   */
  yac::Time t0() const { return t0_; }

  /**
   * Get the end time.
   */
  yac::Time t1() const { return t1_; }
  ...
  bool EvaluateWithMinimalJacobians(double const *const *parameters,
    ...
    // Get speed and bias
    yac::SpeedAndBias speedAndBiases_0;
    yac::SpeedAndBias speedAndBiases_1;
...
#if EST_TIMEDELAY == 1
    yac::Time time_start = t0_;
    yac::Time time_end = t1_;
    // -- Modify t0 and t1
    const yac::Time delay(time_delay);
...
//           // resuse the ImuError2 class and find the jacobians for time_delay.
...
//           const ImuError2 imu0{imuMeasurements_, imuParameters_, t0_, t1_};
//           const ImuError2 imu1{imuMeasurements_, imuParameters_, t0_, t1_};
...
// #endif // if EST_TIMEDELAY == 1
   ...
   */
  virtual std::string typeInfo() const { return "ImuError2"; }

  // Parameters
  ///< The IMU parameters.
  yac::ImuParameters imuParameters_;

  // Measurements
  ///< The IMU measurements used. Must be spanning t0_ - t1_.
  yac::ImuMeasurementDeque imuMeasurements_;

  // Times
  yac::Time t0_; ///< The start time (i.e. time of the first set of states).
  yac::Time t1_; ///< The end time (i.e. time of the sedond set of states).
