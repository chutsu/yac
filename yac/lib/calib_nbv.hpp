...
//   const auto z_FO = fx * target_half_width / (target_half_resolution_x *
//   scale);
//
...
//
//   // Rotate the camera around to see if camera can actually observe the
//   target const vec3_t rpy = deg2rad(vec3_t{-180.0, 0.0, 0.0}); const mat3_t
//   C_FC = euler321(rpy); const mat4_t T_FC = tf(C_FC, r_FO); if
//   (check_fully_observable<CAMERA>(target, cam_params, T_FC) == false) {
//     scale -= 0.1;
//     retry--;
//
//     if (retry == 0) {
...
//     grids.push_back(calib_simulate<CAMERA>(target, T_FC0, cam_params,
//     T_C0Ci));
//   }
//
//   return grids;
