...
#include "autocal/common/Time.hpp"
 ...
 */
void srl_walltime(uint32_t &sec, uint32_t &nsec)
#ifndef WIN32
 ...
 */
int srl_nanosleep(const uint32_t &sec, const uint32_t &nsec) {
#if defined(WIN32)
  HANDLE timer = NULL;
 ...
 */
bool srl_wallsleep(uint32_t sec, uint32_t nsec) {
#if defined(WIN32)
  srl_nanosleep(sec, nsec);
#else
  timespec req = {sec, nsec};
  timespec rem = {0, 0};
...
bool Time::waitForValid() { return waitForValid(autocal::WallDuration()); }

bool Time::waitForValid(const autocal::WallDuration & /*timeout*/) {
  /*autocal::WallTime start = ros::WallTime::now();
  while (!isValid() && !stopped_) {
    autocal::WallDuration(0.01).sleep();
    if (timeout > autocal::WallDuration(0, 0) && (autocal::WallTime::now() -
  start > timeout)) { return false;
    }
  }
  if (stopped_) {
    return false;
...
bool Duration::sleep() const {
  if (Time::useSystemTime()) {
    ...
    while ((Time::now() < end)) {
      srl_wallsleep(0, 1000000);

      // If we started at time 0 wait for the first actual time to arrive before
      ...
      // If time jumped backwards from when we started sleeping, return
      // immediately
      if (Time::now() < start) {
        return false;
      }
...
bool WallDuration::sleep() const { return srl_wallsleep(sec, nsec); }
