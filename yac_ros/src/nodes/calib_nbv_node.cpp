...
void draw_status_text(const std::string &txt, cv::Mat &image) {
  ...
  const float txt_scale = 4.0;
  const int txt_thickness = 4;
  const cv::Scalar txt_color{0, 255, 0};
  int baseline = 0;
  const auto txt_size =
      cv::getTextSize(txt, txt_font, txt_scale, txt_thickness, &baseline);
  int txt_x = (img_w - txt_size.width) / 2.0;
  int txt_y = (img_h + txt_size.height) / 2.0;
  const cv::Point txt_pos{txt_x, txt_y}; // Bottom left of txt string

  // Overlay properties
  const int pad = 20;
  const double alpha = 0.5;
  const int overlay_x1 = ((img_w - txt_size.width) / 2.0) - pad / 2.0;
  const int overlay_y1 = ((img_h - txt_size.height) / 2.0) - pad / 2.0;
  const int overlay_x2 = overlay_x1 + txt_size.width + pad / 2.0;
  const int overlay_y2 = overlay_y1 + txt_size.height + pad / 2.0;
...
struct calib_nbv_node_t {
  ...
  calib_nbv_node_t(const std::string &node_name_) : node_name{node_name_} {
    // Load config
    ...
    // cam0_sub =
    //     ros_nh.subscribe(cam0_topic, 1, &calib_nbv_node_t::image_cb, this);
    // ros_topic_subscribed(cam0_sub, cam0_topic);
  }

  ~calib_nbv_node_t() { tcsetattr(0, TCSANOW, &term_config_orig); }
...
int main(int argc, char *argv[]) {
  ...
  // Start calibrating
  yac::calib_nbv_node_t calib{node_name};
  calib.loop();
