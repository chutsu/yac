...
int solver_t::estimate_covariance_determinant(
  ...
  // spqr; spqr.compute(Hmm.sparseView()); if (spqr.info() != Eigen::Success) {
  //   FATAL("SparseQR decomp failed!");
  // }
  // const matx_t Hmm_inv = spqr.solve(I(m, m));
  // if (spqr.info() != Eigen::Success) {
  ...
  // //     LOG_WARN("Covariance estimation: Inverse identity check: %f",
  // //     inv_check); LOG_WARN("This is bad ... Usually means covariance
  // //     estimated is bad!");
  // //   }
  // //   return -1;
  // // }
  ...
  // // clang-format off
  // Eigen::JacobiSVD<matx_t> svd(H_marg, Eigen::ComputeFullU |
  // Eigen::ComputeFullV); const auto rank = svd.rank(); const vecx_t &s =
  // svd.singularValues(); covar_det = -1.0 * s.head(rank).array().log().sum();
  // // clang-format on
