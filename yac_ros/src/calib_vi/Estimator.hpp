...
class Estimator {
 public:
  ...
  std::map<int, uint64_t> sensor_camera_pose_ids_;
  ...
  std::map<int, std::shared_ptr<PoseParameterBlock>> T_SC_blocks_;
  ...
  uint64_t addImuTimeDelay(const double time_delay, const bool add_prior=false);
  uint64_t addFiducialPose(const mat4_t &T_WF, const bool add_prior=false);
  uint64_t addCamera(const size_t cam_idx,
                     const PinholeRadtan &camera,
                     const bool add_prior=false,
                     const bool fixed=false);
  uint64_t addSensorCameraExtrinsics(const size_t cam_id, const mat4_t &T_SC, const bool add_prior=false);
  ...
  mat4_t getSensorCameraPoseEstimate(const size_t cam_idx);
  ...
  void addReprojErrors(std::map<int, aprilgrid_t> &cam_grids,
                       const size_t pose_id,
                       std::vector<::ceres::ResidualBlockId> &reproj_error_ids,
                       std::vector<std::vector<uint64_t>> &reproj_error_param_ids);
  ::ceres::ResidualBlockId addSpeedBiasError(const SpeedAndBias &sb, uint64_t sb1_id);

	int estimateRelPose(const aprilgrid_t &grid, mat4_t &T_C0F);
